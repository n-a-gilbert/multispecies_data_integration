...
sim_key <- bind_rows(replicate(nsimreps, scenarios, simplify = FALSE)) %>%
  add_column(simrep = 1:nrow(.))
...
for( i in min_index:max_index ) {
  ...
  alpha0_diff[[i]] <- MCMCpstr(out, params = "alpha0_diff", type = "chains" )[[1]] %>% 
    as_tibble(rownames = "sp") %>% 
    pivot_longer(2:ncol(.),
                 names_to = "iter",
                 values_to = "value") %>% 
    mutate(sp = str_replace(sp, "alpha0_diff", "")) %>% 
    mutate(sp = parse_number(sp), 
           iter = parse_number(iter)) %>% 
    add_column(param = "alpha0_diff", 
               simrep = i) %>% 
  ...
  alpha1_diff[[i]] <- MCMCpstr(out, params = "alpha1_diff", type = "chains" )[[1]] %>% 
    ...
    pivot_longer(2:ncol(.),
                 names_to = "iter",
                 values_to = "value") %>% 
    mutate(sp = str_replace(sp, "alpha1_diff", "")) %>% 
    mutate(sp = parse_number(sp), 
           iter = parse_number(iter)) %>% 
  ...
  beta0_diff[[i]] <- MCMCpstr(out, params = "beta0_diff", type = "chains" )[[1]] %>% 
    as_tibble(rownames = "sp") %>% 
    pivot_longer(2:ncol(.),
                 names_to = "iter",
                 values_to = "value") %>% 
    mutate(sp = str_replace(sp, "beta0_diff", "")) %>% 
    mutate(sp = parse_number(sp), 
           iter = parse_number(iter)) %>% 
    add_column(param = "beta0_diff", 
               simrep = i) %>% 
    dplyr::select(simrep, sp, iter, param, value) 
  ...
  N_RB_TC[[i]] <- MCMCpstr(out, params = "N_RB_TC", type = "chains" )[[1]] %>% 
    ...
    pivot_longer(2:ncol(.),
                 names_to = "iter",
                 values_to = "value") %>% 
    mutate(sp = str_replace(sp, "N_RB_TC", "")) %>% 
    mutate(sp = parse_number(sp), 
           iter = parse_number(iter)) %>% 
  ...
  N_RB_DS[[i]] <- MCMCpstr(out, params = "N_RB_DS", type = "chains" )[[1]] %>% 
    ...
    pivot_longer(2:ncol(.),
                 names_to = "iter",
                 values_to = "value") %>% 
    mutate(sp = str_replace(sp, "N_RB_DS", "")) %>% 
    mutate(sp = parse_number(sp), 
           iter = parse_number(iter)) %>% 

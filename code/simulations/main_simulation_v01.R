...
# 100 replicates each of 4 scenarios 
#   1) Equal amounts of data, same detectability
#   2) Equal amounts of data, count detectability 20% lower
#   3) 4x more count data, same detectability
#   4) 4x more count data, count detectability 20% lower
#
# I split this into several different jobs to run on a supercomputer;
# Running all replicates of just 1 scenario took ~5 days, so recommend running just portions
...
# Updated variable names to match the manuscript and hopefully be more reader friendly
model.code <- nimble::nimbleCode({
  ...
  for(i in 1:NCOUNTS) {
    ...
    NG_tc[i] <- exp(alpha0[SP_TC[i]] + alpha1[SP_TC[i]] * HAB_TC[i]) * rho_tc[i]
    GS_tc[i] <- exp(beta0[SP_TC[i]] ) * phi_tc[i]
    N[i] ~ dpois( NG_tc[i] * GS_tc[i] )
    yN[i] ~ dbin( pie_sp[SP_TC[i]], N[i] )
...
sim_key <- dplyr::bind_rows(replicate(nsimreps, scenarios, simplify = FALSE)) |>
  ( function(x) tibble::add_column(x, simrep = 1:nrow(x)))()

alpha0_diff <- list(list())
alpha1_diff <- list(list())
beta0_diff <- list(list())
N_RB_TC <- list(list())
N_RB_DS <- list(list())
sp_info_res <- list(list())
sim_dat <- list(list())

# can break up into separate jobs to go faster
min_index <- 1
max_index <- nrow(sim_key)
...
for( i in min_index:max_index ) {
  ...
  sim_dat[[i]] <- sim_icm(
    nsp = 20,
    nsites = 50,
    nsites_tc_fact = sim_key[[i, "nsites_tc_fact"]],
    p_bias = sim_key[[i, "p_bias"]])
  
  data <- sim_dat[[i]]$data
  constants <- sim_dat[[i]]$constants
  sp_info <- sim_dat[[i]]$sp_info
  ...
  inits <- function(){ 
    list(
      ...
      gamma0 = rnorm(constants$NSPECIES, 5.5, 0.5),
      alpha0 = runif(constants$NSPECIES, 1, 2), 
      alpha1 = rnorm(constants$NSPECIES, 0, 1),
      beta0 = runif(constants$NSPECIES, 1, 2),
      zeta = runif(constants$NSPECIES, 0.2, 0.8),
      xi = runif(constants$NSPECIES, 0.2, 0.8),
      ...
      phi_ng = rgamma(constants$NSURVEYS, 2, 2),
  ...
  sp_info_res[[i]] <- sp_info |> 
    tibble::add_column(simrep = i)
  
  alpha0_diff[[i]] <- MCMCvis::MCMCpstr(out, params = "alpha0_diff", type = "chains" )[[1]] |> 
    tibble::as_tibble(rownames = "sp") |> 
    ( function(x) tidyr::pivot_longer(x,
                                      2:ncol(x),
                                      names_to = "iter",
                                      values_to = "value"))() |> 
    dplyr::mutate(sp = stringr::str_replace(sp, "alpha0_diff", "")) |> 
    dplyr::mutate(sp = readr::parse_number(sp), 
                  iter = readr::parse_number(iter)) |> 
    tibble::add_column(param = "alpha0_diff", 
                       simrep = i) |> 
  ...
  beta0_diff[[i]] <- MCMCvis::MCMCpstr(out, params = "beta0_diff", type = "chains" )[[1]] |> 
    tibble::as_tibble(rownames = "sp") |> 
    ( function(x) tidyr::pivot_longer(x,
                                      2:ncol(x),
                                      names_to = "iter",
                                      values_to = "value"))() |> 
    dplyr::mutate(sp = stringr::str_replace(sp, "beta0_diff", "")) |> 
    dplyr::mutate(sp = readr::parse_number(sp), 
                  iter = readr::parse_number(iter)) |> 
    tibble::add_column(param = "beta0_diff", 
                       simrep = i) |> 
    dplyr::select(simrep, sp, iter, param, value) 
  
  N_RB_TC[[i]] <- MCMCvis::MCMCpstr(out, params = "N_RB_TC", type = "chains" )[[1]] |> 
    tibble::as_tibble(rownames = "sp") |> 
    ( function(x) tidyr::pivot_longer(x,
                                      2:ncol(x),
                                      names_to = "iter",
                                      values_to = "value"))() |> 
    dplyr::mutate(sp = stringr::str_replace(sp, "N_RB_TC", "")) |> 
    dplyr::mutate(sp = readr::parse_number(sp), 
                  iter = readr::parse_number(iter)) |> 
    tibble::add_column(param = "N_RB_TC", 
                       simrep = i) |> 
    dplyr::select(simrep, sp, iter, param, value) 
  ...
  rm( data, constants, sp_info)
  print(paste("Simrep", i, "took", round(time[[1]], 3), "hours"))
...
save(
  ...
  N_RB_DS,
  sp_info_res,
  file = paste0( "simulation_results_simreps_",
                 min_index,
                 "-",
                 max_index,
                 "_",

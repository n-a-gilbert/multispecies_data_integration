...
# see main simulation for comments / more detail
sim_icm <- function(
  ...
  # filter only rare species - those with at least 1 distance observation that have the lowest observed DS count
  ng_data <- data |> 
    dplyr::filter(gs > 0) |> 
...
# this is very similar, just community-level parameters are omitted and species loop is length 1
...
for( i in min(simrep_rank):max(simrep_rank)){
  ...
  readr::write_csv(res, paste0("iss_rare_no_od_simrep_", formatC(simrep_raw[i], width = 4, format = "d", flag = "0"), "_results.csv"))

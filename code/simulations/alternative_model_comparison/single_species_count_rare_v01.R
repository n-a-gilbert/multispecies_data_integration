# simulation script for single-species count only model for rare species
library(tidyverse)
library(nimble)
library(parallel)
library(MCMCvis)
...
# see main simulation script for agonizing details
sim_icm <- function(
  ...
  # filter out only rare species - species observed at least once with minimim sum of observed counts
  transect_counts <- data_c |> 
    ...
    dplyr::ungroup() |> 
    dplyr::filter( num_obs == min(num_obs)) |> 
    dplyr::arrange(sp, site, rep) |> 
...
# see main simulation for more details
...
for( i in min(simrep_rank):max(simrep_rank)){
  ...
  out <- parallel::clusterEvalQ(cl, {
    ...
    modelConf <- cnimble::onfigureMCMC(model)
  ...
  readr::write_csv(res, paste0("ssc_rare_no_od_simrep_", formatC(simrep_raw[i], width = 4, format = "d", flag = "0"), "_results.csv"))

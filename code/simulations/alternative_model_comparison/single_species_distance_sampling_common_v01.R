# single-species, distance-sampling-only model for common species
library(tidyverse)
library(nimble)
library(parallel)
library(MCMCvis)
...
# see main simulation script for more details
sim_icm <- function(
  ...
  # filter out only the most common species
  # defined as the species with the maximum sum of observed counts
  ng_data <- data |> 
    dplyr::filter(gs > 0) |> 
...
# count submodel is omitted, as are community-level parameters
...
for( i in min(simrep_rank):max(simrep_rank)){
  ...
  readr::write_csv(res, paste0("ssds_common_no_od_simrep_", formatC(simrep_raw[i], width = 4, format = "d", flag = "0"), "_results.csv"))

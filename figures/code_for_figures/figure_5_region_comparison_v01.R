...
community <- MCMCvis::MCMCsummary( out, params = c( "mu_alpha0_contrast", 
  ...
  dplyr::mutate( type = base::ifelse( base::grepl("alpha", param),
                                      "Average: number of groups", "Average: group size")) %>% 
  dplyr::mutate( type = base::factor( type, levels = c("Average: number of groups", "Average: group size"))) %>% 
...
species <- MCMCvis::MCMCsummary(out, params = c("alpha0_contrast", "beta0_contrast")) %>% 
  ...
  dplyr::mutate( type = base::ifelse(grepl("alpha", param), "Average: number of groups", "Average: group size")) %>% 
  dplyr::mutate(type = base::factor(type, levels = c("Average: number of groups", "Average: group size"))) %>% 

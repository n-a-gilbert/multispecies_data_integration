...
com_params <- all |> 
  ...
  dplyr::summarise(rb = sum(diff) / sum( abs( truth) ) ) |> 
  ...
  dplyr::select(model, type, param, rb)
...
rb_table <- all |> 
  ...
  dplyr::ungroup() |> 
  dplyr::select(-type) |> 
  dplyr::mutate( rb = paste0( sprintf("%.2f", round( rb * 100, 2)), "%")) |> 
  ...
  tidyr::pivot_wider(names_from = model, values_from = rb) |> 
